...
describe('SnippetComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    TestBed.configureTestingModule({
      imports: [MatDialogModule, MatMenuModule, BrowserAnimationsModule, SnippetComponent],
      providers: [
  ...
  it('should add citation from comment', waitForAsync(() => {
    ...
    let snippet_input = {
      ...
      readACL: ['p20216'],
      updateACL: ['p20216'],
      deleteACL: ['slspollux-nanoxas'],
      shareACL: ['slspollux-nanoxas', 'p20216'],
      adminACL: ['slspollux-nanoxas'],
      tags: [],
      versionable: true,
      deleted: false,
      linkType: 'comment',
  ...
  it('should release the lock', async () => {
    ...
    expect(component['logbookItemDataService'].deleteAllInProgressEditing).toHaveBeenCalledOnceWith(
      snippetMock.id,
    );
  });
  ...
  it('should set locked', () => {
    spyOn(component, 'getLastEditedSnippet').and.returnValue({
    ...
    component.setLocked();
    expect(component.lockEditUntilTimeout).toHaveBeenCalledOnceWith(
      'aUser',
      component._timeoutMilliseconds - 1,
    );
  });
