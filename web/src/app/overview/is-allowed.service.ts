...
export class IsAllowedService {
  ...
  isNotExpired() {
    ...
    let expiresString = `(${expiresAt.toLocaleDateString('en-GB', {
      year: '2-digit',
      month: '2-digit',
      day: '2-digit',
      hour: '2-digit',
  ...
  private otherEnabledMembers(action: string): string[] {
    const aclMembers = (this.snippet?.[`${action}ACL`] ?? [])
      .filter((m: string) => m != 'admin')
      .concat('admin');
    if (
      this.userPreferences.userInfo?.roles?.some((entry: string) => aclMembers?.includes?.(entry))
    )
      return [];
    return aclMembers;
  }
  ...
  isUserAllowed(action: string) {
    ...
    if (aclMembers.length === 0) return true;
    this.tooltips[action] = `Enabled for members of '${aclMembers}'`;
    return false;
  }

  isAnyEditAllowed() {
    const groups = ['update', 'delete'].reduce(
      (previousValue, currentValue) => previousValue.concat(this.otherEnabledMembers(currentValue)),
      [],
    );
  ...
  canChangeOwnerGroup() {
    ...
    if (!this.snippet.ownerGroup || adminMembers.length === 0) return true;
  ...
  private cascadeExpiration(action: string) {
    ...
    else if (!this.tooltips.expired) return this.isUserAllowed(action);
