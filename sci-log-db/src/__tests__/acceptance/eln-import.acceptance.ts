...
function expectArchiveValidationError(
  errorResponse: AnyObject,
  expectedErrors: ElnError[],
): void {
  expect(errorResponse).to.containDeep({
    statusCode: 422,
    name: 'UnprocessableEntityError',
    message: 'Archive validation failed',
  });
  expect(errorResponse.details).to.have.length(expectedErrors.length);
  expect(errorResponse.details).to.containDeep(expectedErrors);
}
